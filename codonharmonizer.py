...
def parse_options():
    ...
    parser = argparse.ArgumentParser(
        usage=usage,
        description='Harmonize your genes for a target organism. See codonfrequencies_from_cds.py to generate frequency files.')

    parser.add_argument(
        "--stats", help="Create harmonized codons statistics files", metavar="")

    input_group = parser.add_argument_group('Required arguments')
    input_group.add_argument("-f", "--fasta",  dest="fasta_filepath",
                             help="DNA (multi)fasta file", required=True, metavar="FASTA")
    input_group.add_argument("-s", "--source", dest="source",
                             help="Source Organism eg. Eco_MG1655")
    input_group.add_argument("-t", "--target", dest="targets",
                             help="Target organism(s) eg. Eco_MG1655. Can be a comma separated list.", required=True, action="append")
    input_group.add_argument("-o", "--output", dest="output_file",
                             help="Output name", required=True,  metavar="NAME")

    inputs = parser.parse_args()
    inputs.output_file = inputs.output_file.strip(".zip")
    inputs.output_file = inputs.output_file.replace(".fasta", "").replace(
        ".zip", "").replace(".fa", "").replace(".fna", "")
...
def get_fasta_sequences(fasta_contents):
    ...
    try:
        ...
        sys.stderr.write(
            "Not a valid DNA fasta file, maybe contains non DNA letters")
        sys.exit()
...
def split_to_codons(sequence, header):
    ...
    if len(sequence) % 3 == 0:
        ...
        sys.stderr.write("Partial sequence >"+header +
...
def get_input_sequence_freq(fasta_contents, freqs):
    ...
    for header in input_sequences:
        ...
        for seq_pos in range(len(AA_seq)):
            codon_pos = [i for i, x in enumerate(
                freqs[AA_seq[seq_pos]][0]) if x == codon_sequence[seq_pos]][0]
            seq_freqs[header].append(
                [codon_sequence[seq_pos], AA_seq[seq_pos], freqs[AA_seq[seq_pos]][1][codon_pos]])
...
def harmonize_sequences(source_gene_freqs, target_freqs):
    ...
    for AA_position in source_gene_freqs:
        ...
        closest_freq = min(enumerate(target_AA_freqs),
                           key=lambda x: abs(x[1]-source_codon_freq))[0]

        harmonized_sequence_freqs.append(
            [target_freqs[AA][0][closest_freq], AA, target_freqs[AA][1][closest_freq]])
...
def main(argv):
    ...
    source_name, source_freqs, source_counts = source_codon_freqs[
        0], source_codon_freqs[1], source_codon_freqs[2]
    ...
    for target in input_targets:
        ...
        target_name, target_freq, target_count = target_codon_freqs[
            0], target_codon_freqs[1], target_codon_freqs[2]

        target_freqs[target_name] = target_freq
        target_counts[target_name] = target_count
        target_gene_freqs[target_name] = get_input_sequence_freq(
            fastafile, target_freqs[target_name])
    ...
    for sequence in source_gene_freqs:
        ...
        for target in target_gene_freqs:
            ...
            harm_gene_freqs = harmonize_sequences(
                source_gene_freqs[sequence], target_freqs[target])

            source_sequence = "".join(
                [AA_pos[0] for AA_pos in source_gene_freqs[sequence]])
            harmonized_sequence = "".join(
                [AA_pos[0] for AA_pos in harm_gene_freqs])
            ...
            non_CHI = get_index_score(
                target_gene_freqs[target][sequence], source_gene_freqs[sequence])
            ...
            harm_out_file.write(",,CHI:,0,,CHI:," +
                                str(non_CHI)+",,CHI:,"+str(CHI)+"\n")
            harm_out_file.write(",,CAI:,"+str(CAI_native)+",,CAI:," +
                                str(CAI_nonNative)+",,CAI:,"+str(CAI_harmonized)+"\n\n")

            harm_out_file.write(","+source_name+" Source gene in "+source_name+",,,"+source_name +
                                " Source gene in "+target+",,,"+source_name+" harmonized gene in "+target+"\n")
            harm_out_file.write(
                "AA pos.,codon,AA,codon freq.,codon,AA,codon freq.,codon,AA,codon freq.\n")
            ...
            for i, source_AA_pos in enumerate(source_gene_freqs[sequence]):
                ...
                harm_out_file.write(
                    target_gene_freqs[target][sequence][i][0]+",")
                harm_out_file.write(
                    target_gene_freqs[target][sequence][i][1]+",")
                harm_out_file.write(
                    str(target_gene_freqs[target][sequence][i][2])+",")
            ...
            harm_out_file.write("\n\n,,"+source_name+",,"+target+"\n")
            harm_out_file.write(
                "AA,codon,source organism freq.,source organism total,target organism freq,target organism total.\n")

            for AA in AAs:
                for i, codon in enumerate(source_counts[AA][0]):
                    # Fix, Needed for matching codon and frequencies in target.
                    target_codon_pos = [c for c, x in enumerate(
                        target_freqs[target][AA][0]) if x == codon][0]

                    harm_out_file.write(AA+","+codon+","+str(source_freqs[AA][1][i])+","+str(source_counts[AA][1][i])+","+AA+","+target_freqs[target][AA][0][target_codon_pos]+","+str(
                        target_freqs[target][AA][1][target_codon_pos])+","+str(target_counts[target][AA][1][target_codon_pos])+"\n")

            fasta_value = ">"+sequence + \
                " codons harmonized [from "+source_name + \
                " for "+target+"]\n"+harmonized_sequence+"\n"
